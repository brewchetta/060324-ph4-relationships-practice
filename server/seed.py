...
if __name__ == '__main__':
    with app.app_context():
        ...
        alb2 = Album(
            title="Is This It", 
            date_of_release=datetime.strptime('7-30-2001', '%m-%d-%Y'),
            artist_id=a2.id
        )

        alb3 = Album(
            title="Room on Fire", 
            date_of_release=datetime.strptime('10-28-2003', '%m-%d-%Y'),
            artist_id=a2.id
        )
        ...
        s2 = Song(title="Some Day", album_id=alb2.id)

        s3 = Song(title="Reptilia", album_id=alb3.id)
